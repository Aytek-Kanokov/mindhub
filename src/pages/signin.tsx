...
function OTPModal({ children }: { children: ReactNode }) {
  ...
  const [updatePasswordURI, setUpdatePasswordURI] = useState('');

  useEffect(() => {
    const url = new URL(window.location.href);
    setUpdatePasswordURI(url.origin + '/update-password');
  }, []);
  ...
  const submitHandlerEmail = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    toast.success('Email sent your way!');
  };
